...
const NewsList = forwardRef<HTMLDivElement, NewsListProps>(({ ...props }, ref) => {
            ...
            case 'error-no-data':
                return (
                    <div className="news-empty">
                        <h1>
                            Si è presentato un errore inaspettato. Non è possibile caricare la lista degli elementi.
                        </h1>
                    </div>
                );
    ...
    async function getNews() {
        const response = await databases.listDocuments(
            ...
            [
                Query.select(['category', 'creationDate', '$id', 'redirectLink', 'subtitle', 'thumbnail', 'title']),
